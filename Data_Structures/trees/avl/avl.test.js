...
function generateTreeWithKeysFrom1Upto(trials) {
...
function removeAndEnsureNodeDoesNotExistAndTreeBalanced(tree, key) {
  //Value to be deleted must be preexisting so as to validate the deletion.
  expect(tree.find(key)).not.toBeNull();
  tree.remove(key);

  //ensure balance after deletion.
  ensureIsBalanced(tree.root());

  // ensure all that the node does not exist.
